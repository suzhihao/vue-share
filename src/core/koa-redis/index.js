...
var RedisStore = module.exports = function (options) {
   ...
   * zhihao_su: 新增prefix
   */
  debug('redis config prefix: %s', options.prefix || (options.prefix = ''));
  if (options.prefix.length > 0 && options.prefix.charAt(options.prefix.length - 1) !== ':') {
    options.prefix += ':';
  }
...
RedisStore.prototype.get = function *(sid) {
  var data = yield this.client.get(this.prefix + sid);
...
RedisStore.prototype.set = function *(sid, sess, ttl) {
  ...
  if (ttl) {
    ...
    yield this.client.setex(this.prefix + sid, ttl, sess);
  } else {
    debug('SET %s %s', sid, sess);
    yield this.client.set(this.prefix + sid, sess);
  }
...
RedisStore.prototype.destroy = function *(sid) {
  debug('DEL %s', sid);
  yield this.client.del(this.prefix + sid);
